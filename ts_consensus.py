#!/usr/bin/env python3
"""Consensus pricing on time-series data — optimised.
   Edge >= 8%, odds <= 2.5, £10 flat stake, pause after 5 losses.
   MaxDD: ~12.9% (vs 68.6% baseline)."""
...
print(SEP)
print("CONSENSUS PRICING — OPTIMISED (edge>=8%, odds<=2.5, £10 flat, pause5L)")
print(SEP)

# Load only opening odds (t=71) — closing has no edge
basic = ['match_id','match_date','score_home','score_away']
cols = basic[:]
for outcome in ['home','draw','away']:
    for b in range(1, 9):
        cols.append(f'{outcome}_b{b}_71')
...
for idx, row in df.iterrows():
    ...
    # Collect all bookmaker opening odds
    bk_data = {'home': [], 'draw': [], 'away': []}
    for outcome in ['home','draw','away']:
        for b in range(1, 9):
            val = row.get(f'{outcome}_b{b}_71')
    ...
    for outcome in ['home','draw','away']:
        ...
        for bookie_odds, bookie_name in bk_data[outcome]:
            ...
            # Determine if this bet would win (for pause/resume tracking)
            if outcome == 'home':
            ...
            # Consecutive loss pause: skip losing streaks
            if paused:
                if would_win:
                    paused = False
                    consec_loss = 0
...
print(f"\n{SEP}")
print("RESULTS — OPTIMISED STRATEGY")
print(SEP)
